...
bool IsCanonicalPubKey(const valtype &vchPubKey) {
    if (vchPubKey.size() != 20)
        return error("Non-canonical public key: length != 20");
    return true;
}
...
bool IsCanonicalSignature(const valtype &vchSig) {
    ...
    if (vchSig.size() != 66)
        return error("Non-canonical signature: length != 66");
...
{
private:
     // sigdata_type is (signature hash, signature, public key hash):
    typedef boost::tuple<uint256, std::vector<unsigned char>, uint160> sigdata_type;
    std::set< sigdata_type> setValid;
    boost::shared_mutex cs_sigcache;

public:
    bool
    Get(const uint256 &hash, const std::vector<unsigned char>& vchSig, const uint160& pubKey)
    {
    ...
    void Set(const uint256 &hash, const std::vector<unsigned char>& vchSig, const uint160& pubKey)
    {
        ...
        while (static_cast<int64>(setValid.size()) > nMaxCacheSize)
        {
            ...
            std::set<sigdata_type>::iterator it =
                setValid.lower_bound(sigdata_type(randomHash, unused, uint160()));
            if (it == setValid.end())
                it = setValid.begin();
            setValid.erase(*it);
        }
              ...
              const CTransaction& txTo, unsigned int nIn, int nHashType, int flags)
{
    ...
    uint160 PubKeyHash(vchPubKey);
    uint256 sighash = SignatureHash(scriptCode, txTo, nIn, nHashType);

    if (signatureCache.Get(sighash, vchSig, PubKeyHash))
        return true;

    CPubKey RecoveredPubKey;
    if (!RecoveredPubKey.RecoverCompact(sighash, vchSig))
        return false;
    if (!RecoveredPubKey.IsValid())
        return false;
    uint160 RecoveredPubKeyHash = Hash160(RecoveredPubKey.begin(), RecoveredPubKey.end());
    if (RecoveredPubKeyHash != PubKeyHash)
        return false;

    if (!(flags & SCRIPT_VERIFY_NOCACHE))
        signatureCache.Set(sighash, vchSig, PubKeyHash);
...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (sTemplates == NULL) {
        ...
        sTemplates = (SScriptPairRec *)malloc(sizeof(SScriptPairRec) * 3);
        
        // order templates such that most common transaction types are checked first
        tScript = new CScript();
        *tScript << OP_PUBKEYHASH << OP_CHECKSIG;
        ...
        *tScript << OP_SMALLINTEGER << OP_PUBKEYS << OP_SMALLINTEGER << OP_CHECKMULTISIG;
        sTemplates[1].txType = TX_MULTISIG;
        sTemplates[1].tScript = tScript;
        
        sTemplates[2].txType = (txnouttype)-1;
        sTemplates[2].tScript = NULL;
    }
