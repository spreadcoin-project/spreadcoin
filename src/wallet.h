...
public:
    ...
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed=true, bool fIncludeLocked=false, const CCoinControl *coinControl=NULL) const;
...
public:
    ...
    (
        ...
        if (!fRead)
        {
            ...
            if (nTimeSmart)
                pthis->mapValue["timesmart"] = strprintf("%" PRI64d, nTimeSmart);
        }
