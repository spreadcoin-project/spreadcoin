...
/** Out stream writting to the buffer with known size.
 */
...
public:
    ...
    explicit CBufferStream(int nTypeIn, int nVersionIn)
    {
        nWritePos = 0;
        nType = nTypeIn;
        nVersion = nVersionIn;
    ...
    // Stream subset
    //
    CBufferStream& write(const char* pch, int nSize)
    {
        // Write to the end of the buffer
        assert(nSize >= 0);
        assert(nWritePos + nSize <= N);
        memcpy(vch + nWritePos, pch, nSize);
        nWritePos += nSize;
        return (*this);
    }

    template<typename T>
    CBufferStream& operator<<(const T& obj)
    {
        // Serialize to this stream
        ::Serialize(*this, obj, nType, nVersion);
        return (*this);
